...
int main()
{
	 ...
	 *  Use pointers when you have a complex type.
	 */
	...
	std::cout << "Distance between " << gameObjects["Ship"]->GetName() << " and "
			  << gameObjects["Enemy"]->GetName() << " is: " << std::to_string(distance) << std::endl;
	...
	if (infile.is_open())
	{
		...
		while(!infile.fail())
		{
			...
			auto* tempObj = new GameObject(name, id, x, y);
			
			gameObjects[name + " 2"] = tempObj;
		}
		infile.close();
	}
